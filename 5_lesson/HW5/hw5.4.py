...
def moves(pl_act): #Распаковка списка
    first, second = players
    return second if pl_act == first else first


players = ["human", 'bot' if int(input('Play with bot 1 - yes, 0 - no? ')) else 'person']#Собираем игроков
shuffle(players) #Перемешиваем

active_player = players[0] #Смотрим кто встал на нулевой индекс
...
while CANDIES > 0:
    ...
    if get_candies not in range(1, CANDIES_LIMIT + 1):
        ...
        if CANDIES > 0:
            ...
            print(f'The player {active_player} won!')#Выиграли
